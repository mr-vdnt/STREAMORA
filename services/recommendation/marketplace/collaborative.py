...
class CollaborativeCandidateGenerator(BaseCandidateGenerator):
    """
    Collaborative filtering generator using implicit user-item interaction signals.
    """
    ...
    async def generate_candidates(
        ...
        with self.repo.get_session() as session:
            stats = session.query(ContentStatistics).order_by(ContentStatistics.popularity.desc()).limit(20).all()
            for s in stats:
                candidates.append(RecommendationCandidateDTO(
                    content_id=s.content_id,
                    generator_name=self.name,
                    score=0.88,
                    reason="Top collaborative item amongst users with similar viewing patterns",
                    provenance_metadata={"popularity_score": s.popularity}
                ))
        return candidates
