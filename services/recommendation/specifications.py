...
class GenreSpecification(Specification):
    ...
    def apply_filter(self, query: Any) -> Any:
        return query.filter(ContentMetadata.overview.ilike(f"%{self.genre}%"))
...
class ThemeSpecification(Specification):
    ...
    def apply_filter(self, query: Any) -> Any:
        return query.filter(ContentMetadata.overview.ilike(f"%{self.theme}%"))
...
class MinRatingSpecification(Specification):
    ...
    def apply_filter(self, query: Any) -> Any:
        return query.filter(ContentStatistics.average_rating >= self.min_rating)
...
class MinPopularitySpecification(Specification):
    ...
    def apply_filter(self, query: Any) -> Any:
        return query.filter(ContentStatistics.popularity >= self.min_popularity)
...
class LanguageSpecification(Specification):
    ...
    def apply_filter(self, query: Any) -> Any:
        return query.filter(ContentMetadata.language.in_(self.languages))
